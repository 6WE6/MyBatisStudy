...
//		session.insert("com.briup.one2one.One2OneMapper.saveHus", hus);
	...
	public void delete_hus(){
		 ...
		 * 第一种方式可以不构建接口的实现类:第一个参数是该接口中的全限定方法名，第二个参数是该接口中的该方法的参数，有参数，写在第二个位置，没有则忽略;最多接收两个参数
		 */
		session.delete("com.briup.one2one.One2OneMapper.deleteHus", 2);
	...
	public void select_husId(){
		try{
		SqlSession session = MyBatisSqlSessionFactory.openSession(true);
		Hus hus = session.selectOne("com.briup.one2one.One2OneMapper.findHusById", 1);
		System.out.println(hus);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public void select_hus(){
		try{
		SqlSession session = MyBatisSqlSessionFactory.openSession(true);
		List<Hus> hus = session.selectList("com.briup.one2one.One2OneMapper.findHus");
		System.out.println(hus);
		}catch(Exception e){
			e.printStackTrace();
		}
